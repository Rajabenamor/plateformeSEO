...
export default function GithubCallbackComponent() {
    ...
    useEffect(() => {
        ...
        const exchangeCode = async () => {
            try {
                const res = await secureFetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/auth/github/exchange/`, {
                    method: "POST",
                    headers: {
                ...
                // Wait briefly, then redirect to integrations or dashboard
                setTimeout(() => {
            ...
            <div className="bg-white/5 border border-white/10 p-8 rounded-2xl max-w-md w-full text-center">
                ...
                {status === "success" && (
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-slate-400 text-sm">Redirecting you back to your integrations...</p>
                    </div>
                )}
